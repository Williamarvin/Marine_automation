...
bool OpManagerEngine::setupOpinionManager(double time)
{
  ...
  double total_opinions = static_cast<double>(m_options.size());
  std::vector<double> setup_opinions; 
  for (int i = 0; i < m_options.size(); i++) {
    setup_opinions.push_back(1.0/total_opinions);  // already checked for divide by 0
    m_opinion_inputs.push_back(0.0); 
  }

  m_opinions = projectOp(setup_opinions);
...
bool OpManagerEngine::receiveOpinionMessage(std::string optn_msg, double msg_time)
{
  ...
  if (ok){
    ...
    // creation of opinion record not ok
    return(false);
  }
...
bool OpManagerEngine::iterateOwnOpinion(double time)
{
  ...
  for (unsigned int j=0; j<options_status.size(); j++){
    ...
    if (!options_status[j]){
      ...
      m_opinions[j] = m_opinions[j] + dt * Z_dot_active[j_offset];
