...
//                    cout << "hello" << endl;
                    Notify(m_nav_prefix+"_LAT", formatDecimal(packet.lat / 10000000, 10), "GPRMC");
                    Notify(m_nav_prefix+"_LON", formatDecimal(packet.lon / 10000000, 10), "GPRMC");
                    Notify(m_nav_prefix+"_LONG", formatDecimal(packet.lon / 10000000, 10), "GPRMC");
                    Notify(m_gps_prefix+"_LAT", formatDecimal(packet.lat / 10000000, 10), "GPRMC");
                    Notify(m_gps_prefix+"_LON", formatDecimal(packet.lon / 10000000, 10), "GPRMC");
                    Notify(m_gps_prefix+"_LONG", formatDecimal(packet.lon / 10000000, 10), "GPRMC");      
                    Notify(m_nav_prefix+"_X", formatDecimal(packet.lat / 10000000, 10), "GPRMC");
                    Notify(m_nav_prefix+"_Y", formatDecimal(packet.lon / 10000000, 10), "GPRMC");
                    Notify(m_gps_prefix+"_X", formatDecimal(packet.lat / 10000000, 10), "GPRMC");
                    Notify(m_gps_prefix+"_Y", formatDecimal(packet.lon / 10000000, 10), "GPRMC");    
                    ...
                    cout <<"GPS Latitude: "; cout << formatDecimal(packet.lat / 10000000, 10);
                    cout <<"GPS Longitude: "; cout << formatDecimal(packet.lon / 10000000, 10);
                    cout <<"GPS Speed: "; cout << packet.vel;
                    // cout <<"Sats Visible: "; cout << packet.satellites_visible;
  		              m_nav_hdg = packet.yaw;
                    m_nav_spd = packet.vel;
                    m_nav_x = stod(formatDecimal(packet.lat / 10000000, 10));
                    m_nav_y = stod(formatDecimal(packet.lon / 10000000, 10));      
 }
...
void M300::receiveCommBeacon(){
    ...
    std::string line(buffer, num_bytes);

    // Find the position of the first space character
    size_t spacePos = line.find(' ');

    // Extract the substring representing the first value
    std::string firstValueStr = line.substr(0, spacePos);

    // Extract the substring representing the second value
    std::string secondValueStr = line.substr(spacePos + 1);

    // Convert the extracted substrings to floating-point values
    double firstValue, secondValue;
    std::istringstream(firstValueStr) >> firstValue;
    std::istringstream(secondValueStr) >> secondValue;

    Notify(m_nav_prefix+"_LAT", to_string(firstValue), "GPRMC");
    Notify(m_nav_prefix+"_LON", to_string(secondValue), "GPRMC");
    Notify(m_nav_prefix+"_LONG", to_string(secondValue), "GPRMC");
    Notify(m_gps_prefix+"_LAT", to_string(firstValue), "GPRMC");
    Notify(m_gps_prefix+"_LON", to_string(secondValue), "GPRMC");
    Notify(m_gps_prefix+"_LONG", to_string(secondValue), "GPRMC");      
    Notify(m_nav_prefix+"_X", to_string(firstValue), "GPRMC");
    Notify(m_nav_prefix+"_Y", to_string(secondValue), "GPRMC");
    Notify(m_gps_prefix+"_X", to_string(firstValue), "GPRMC");
    Notify(m_gps_prefix+"_Y", to_string(secondValue), "GPRMC");  
...
bool M300::Iterate()
{
  ...
  if(pik_port >= 0 && mode == "floatie") {
      sendMessagesToSocket();
      receiveCommFloatie();
  }
  else if(pik_port >= 0 && mode == "beacon"){
    receiveCommBeacon();
    //sendMessagesToSocket();
  }
  else{
