...
///////////////////////////////////////////////////////
  ...
  if(m_mode=="rotate"||m_mode=="rotate_speed"){
	  ...
	  // generate a random percentage change in radius
	  int rand_pct_rad_max = 10;
          int rand_pct_rad =( rand() % rand_pct_rad_max + 1);
	  double rand_pct_rad_dbl = double(rand_pct_rad);
	  rand_pct_rad_dbl = rand_pct_rad_dbl / 100.0 + 1.0;
          ...
          if (dis>min_rad){
            ...
            expanding = 100*expanding;
            crs_zaic.setSummit( (resultt+expanding) * rand_pct_rad_dbl );
          }
          else if(min_rad-dis<1.5){
            crs_zaic.setSummit(resultt * rand_pct_rad_dbl );
          }
          else{
            crs_zaic.setSummit( (resultt-20) * rand_pct_rad_dbl);
          }
...
/////////////////////////////////////////////////////////
  ...
  //////////////////////////////////////////////////////
    ...
    //and drive toward the center of the region
        double dx1=5*speedord*sin(m_rand_heading*M_PI/180);
        double dy1=5*speedord*cos(m_rand_heading*M_PI/180);
    ...
    if(m_op_region.contains(p_osx1, p_osy1)){
      ...
      bool in_region=true;
      if(m_outbound){
        double dx=14*speedord*sin(m_rand_heading*M_PI/180);
        double dy=14*speedord*cos(m_rand_heading*M_PI/180);
        double p_osx=m_osx+dx;
        double p_osy=m_osy+dy;
        in_region = m_op_region.contains(p_osx, p_osy);
      }
