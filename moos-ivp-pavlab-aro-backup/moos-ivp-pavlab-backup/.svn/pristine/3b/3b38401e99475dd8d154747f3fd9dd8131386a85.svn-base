...
FldBloomStormSim::FldBloomStormSim()
{
   ...
   m_unsampled_blooms_completed = 0; 
...
bool FldBloomStormSim::checkActiveBlooms()
{
  ...
  for (it = m_active_blooms.begin(); it != m_active_blooms.end();){
    ...
    if ((MOOSTime() - it->get_time()) > it->get_duration()){
      ...
      if (m_blooms_sampled_labels.count(it->get_label()) == 0) {
	// it was not sampled
	m_unsampled_blooms_completed += 1;
      } else {
	m_sampled_blooms_completed += 1; 
      }
      
      Notify("UNSAMPLED_BLOOMS_COMPLETED", uintToString(m_unsampled_blooms_completed));  
...
bool FldBloomStormSim::notifyVehiclesOfBloom()
{
  ...
  for (it2 = m_active_blooms.begin(); it2 != m_active_blooms.end(); it2++) {
 	...
 	/*
    vector<double> bloom_circle = it2->get_circle(MOOSTime(), 100);
    XYPolygon bloom_poly;
    for (unsigned int i=0; i<bloom_circle.size()-1;){
      bloom_poly.add_vertex(bloom_circle[i], bloom_circle[i+1], false);
      i+=2; 
    }

    bloom_poly.determine_convexity();
    if ( !bloom_poly.is_convex() ) {
      continue; 
    }
    Notify("TMP_DEBUG", "built polygon for " + it2->get_spec()); 
	*/
	...
	//Notify("TMP_DEBUG", "checking polygon contains "); 
	//if (bloom_poly.contains(vx, vy)) {
	//  m_vehicles_in_bloom.insert(it->first); 
	//}
      }
    }
...
bool FldBloomStormSim::manageStorm()
{
  ...
  /*
  if (m_storm_heading > 360.0) {
    m_storm_heading -= 360.0;
  } else if (m_storm_heading < -360.0) {
    m_storm_heading += 360.0;
  }
  */
...
bool FldBloomStormSim::handleSampleFinishedLog(const std::string vname)
{
  ...
  if (it != m_map_node_records.end()) {
	...
	i+=2; 
      ...
      bloom_poly.determine_convexity();
      // Continue on if there are problems. 
      if ( !bloom_poly.is_convex() ) {
	continue; 
      }
...
bool FldBloomStormSim::buildReport() 
{
  ...
  m_msgs << bloom_list_str << endl;
   ...
   m_msgs << " Completed Blooms: Sampled: " << uintToString(m_sampled_blooms_completed) << " Unsampled: " << uintToString(m_unsampled_blooms_completed) << endl;
