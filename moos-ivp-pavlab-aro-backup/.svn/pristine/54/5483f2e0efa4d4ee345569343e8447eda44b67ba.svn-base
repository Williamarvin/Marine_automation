...
void CollisionDetector::handleCPAEvent(CPAEvent event)
{
  ...
  string situation = event.getCOLREGSsituation(); //added
  ...
  if(situation == "overtaking"){
    m_total_overtaking++;
    m_map_vname_overtaking[v1]++;
    m_map_vname_overtaking[v2]++;
    Notify("OVERTAKING_TOTAL", m_total_overtaking);
  }
  else if(situation == "headon"){
    m_total_headon++;
    m_map_vname_headon[v1]++;
    m_map_vname_headon[v2]++;
    Notify("HEADON_TOTAL", m_total_headon);
  }
  else if(situation == "crossing"){
    m_total_crossing++;
    m_map_vname_crossing[v1]++;
    m_map_vname_crossing[v2]++;
    Notify("CROSSING_TOTAL", m_total_crossing);
  }
  
  string event_str = "Encounter: " + v1 + " and " + v2 + ":";
  event_str += "rank=" + rank + ", cpa=" + cpas + ", situation=" + situation; 
...
void CollisionDetector::handleMailNodeReport(string sval)
{
  ...
  // Part 3: The first time we deal with a particular vehicle, we 
...
bool CollisionDetector::buildReport() 
{
  ...
  for(p=m_map_vname_encounters.begin(); p!=m_map_vname_encounters.end(); p++) {
    ...
    unsigned int overtaking  = m_map_vname_overtaking[vname];
    unsigned int headon      = m_map_vname_headon[vname];
    unsigned int crossing    = m_map_vname_crossing[vname];
    ...
    string s_overtaking  = uintToString(overtaking);
