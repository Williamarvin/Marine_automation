...
bool NodeReporter::OnNewMail(MOOSMSG_LIST &NewMail)
{
  ...
  for(p=NewMail.begin(); p!=NewMail.end(); p++) {
    ...
    if(key == "NAV_X") {
      m_record.setX(stod(sdata));
      m_nav_xy_updated = m_curr_time;
    }
    else if(key == "NAV_Y") {
      m_record.setY(stod(sdata));
      m_nav_xy_updated = m_curr_time;
    }
    else if(key == "NAV_LAT") {
      m_record.setLat(stod(sdata));
      m_nav_latlon_updated = m_curr_time;
    }
    else if(key == "NAV_LONG") {
      m_record.setLon(stod(sdata));
      m_nav_latlon_updated = m_curr_time;
    }
    else if(key == "NAV_SPEED") 
      m_record.setSpeed(stod(sdata));
    else if(key == "NAV_HEADING")
      m_record.setHeading(angle360(stod(sdata)));
